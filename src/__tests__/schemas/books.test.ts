...
describe("Books Collection Schema", () => {
  ...
  describe("URL validation", () => {
    it("rejects executable schemes in buy links", async (): Promise<void> => {
      const { booksSchema } = await import("@/schemas/blog");
      const book = {
        title: "Test",
        post_slug: "test",
        ...
        book_card: "https://example.com/book-card",
      };

      expect(booksSchema.safeParse(book).success).toBe(false);
    });

    it("rejects executable schemes in book cards", async (): Promise<void> => {
      const { booksSchema } = await import("@/schemas/blog");
      const book = {
        title: "Test",
        post_slug: "test",
        ...
        score: 3,
        author: "test",
        buy: [{ type: "ebook", link: "https://example.com/buy" }],
