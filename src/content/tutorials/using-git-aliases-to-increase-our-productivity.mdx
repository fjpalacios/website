...
```
As you can see, preceded by the symbol **!** as we said before (it's true that *everything are Git commands*, but we are using the `&&` typical symbol of the command line to be able to run several commands from a single alias, so technically they aren't Git commands anymore). But we'll also give you some idea to create aliases with Git commands, such as the `git co master` or` git cb sidebar-feature` commands that we are just now going to create:
```bash
