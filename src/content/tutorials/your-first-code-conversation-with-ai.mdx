...
One thing to understand from the start: **each conversation is independent**. The model has no memory of previous sessions. Everything we covered in earlier tutorials about hallucinations and context blindness applies here: it only knows what's in _this_ conversation.
...
That gap — between correct generation and _appropriate_ generation — is the one that bites you in production. The first is the AI's job. The second is yours.
